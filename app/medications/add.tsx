...
const FREQUENCY_LABELS: { [key: number]: { label: string; range: string }[] } = {
  1: [
    { label: "Time", range: "" },
  ],
  2: [
    { label: "Morning", range: "6:00–10:00 AM" },
    { label: "Night", range: "7:00–10:00 PM" },
  ],
  3: [
    { label: "Morning", range: "6:00–10:00 AM" },
    { label: "Afternoon", range: "12:00–3:00 PM" },
    { label: "Night", range: "7:00–10:00 PM" },
  ],
  4: [
    { label: "Morning", range: "6:00–10:00 AM" },
    { label: "Noon", range: "11:00 AM–1:00 PM" },
    { label: "Evening", range: "4:00–6:00 PM" },
    { label: "Night", range: "7:00–10:00 PM" },
  ],
};

const FREQUENCY_DEFAULT_TIMES: { [key: number]: string[] } = {
  1: ["08:00"], // Once daily: 8 AM
  2: ["08:00", "20:00"], // Twice daily: 8 AM, 8 PM
  3: ["08:00", "14:00", "20:00"], // Three times daily: 8 AM, 2 PM, 8 PM
  4: ["08:00", "12:00", "16:00", "20:00"], // Four times daily: 8 AM, 12 PM, 4 PM, 8 PM
};
...
export default function AddMedicationScreen() {
  ...
  const handleFrequencySelect = (freq: string) => {
    ...
    let times: string[] = [];
    if (selectedFreq) {
      const freqCount = selectedFreq.times.length > 0 ? selectedFreq.times.length : 1;
      times = FREQUENCY_DEFAULT_TIMES[freqCount] || ["08:00"];
    }
    setForm((prev) => ({
      ...prev,
      frequency: freq,
          ...
          <View style={styles.section}>
            ...
            {form.frequency && form.frequency !== "As needed" && (
              <View style={styles.timesContainer}>
                ...
                {(function() {
                  const selectedFreq = FREQUENCIES.find(f => f.label === form.frequency);
                  const freqCount = selectedFreq ? (selectedFreq.times.length > 0 ? selectedFreq.times.length : 1) : 1;
                  const slots = FREQUENCY_LABELS[freqCount] || [{ label: "Time", range: "" }];
                  return slots.map((slot, index) => (
                    ...
                    >
                      ...
                      <View style={{ flex: 1 }}>
                        <Text style={styles.timeLabel}>{slot.label}{slot.label === 'Time' ? '' : (slot.range ? ' (' + slot.range + ')' : '')}</Text>
                        <Text style={styles.timeButtonText}>{form.times[index]}</Text>
                      </View>
                      <Ionicons name="chevron-forward" size={20} color="#666" />
                    </TouchableOpacity>
                  ));
            ...
            {showTimePickerIndex !== null && (() => {
              ...
              const freqCount = selectedFreq ? (selectedFreq.times.length > 0 ? selectedFreq.times.length : 1) : 1;
              const slots = FREQUENCY_LABELS[freqCount] || [{ label: "Time", range: "" }];
              const slot = slots[showTimePickerIndex!];
              // Parse range
              let minHour = 0, minMinute = 0, maxHour = 23, maxMinute = 59;
              if (slot.label !== "Time" && slot.range) {
                // Example: "6:00–10:00 AM" or "7:00–10:00 PM"
                const rangeParts = slot.range.split("–");
                if (rangeParts.length === 2) {
                  ...
                  const min = parseTime(rangeParts[0]);
                  const max = parseTime(rangeParts[1]);
                  minHour = min.h; minMinute = min.m;
                  maxHour = max.h; maxMinute = max.m;
                }
              }
              return (
                <DateTimePicker
                  value={(() => {
                    const t = form.times[showTimePickerIndex!];
                    const [h, m] = t ? t.split(":") : ["08", "00"];
                    const d = new Date();
                    d.setHours(Number(h), Number(m), 0, 0);
                    return d;
                  })()}
                  mode="time"
                  minimumDate={slot.label !== "Time" ? (() => { const d = new Date(); d.setHours(minHour, minMinute, 0, 0); return d; })() : undefined}
                  maximumDate={slot.label !== "Time" ? (() => { const d = new Date(); d.setHours(maxHour, maxMinute, 0, 0); return d; })() : undefined}
                  onChange={(event, date) => {
                    ...
                    if (date) {
                      ...
                      setForm((prev) => {
                        ...
                        newTimes[showTimePickerIndex!] = newTime;
                        return {
                          ...prev,
                          times: newTimes,
                        };
